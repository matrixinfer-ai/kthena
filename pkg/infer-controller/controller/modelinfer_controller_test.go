...
func TestIsInferGroupPodNumExpected(t *testing.T) {
	ns := "test-ns"
	groupName := "test-group"
	expectedRevision := "hash123"
	otherRevision := "otherhash"
	group := datastore.InferGroup{
		Name:     groupName,
	...
	cases := []struct {
		...
		{
			name:          "no pods, expect 0",
			pods:          nil,
			expectedCount: 0,
			want:          true,
		},
		{
			name:          "no pods, expect 1",
			pods:          nil,
			expectedCount: 1,
			want:          false,
		},
		{
			name: "all revision match, count match",
			pods: []podSpec{
				{"pod1", map[string]string{
					workloadv1alpha1.GroupNameLabelKey: groupName,
					workloadv1alpha1.RevisionLabelKey:  expectedRevision,
				}},
		...
		{
			name: "all revision match, count not match",
			pods: []podSpec{
				{"pod1", map[string]string{
					workloadv1alpha1.GroupNameLabelKey: groupName,
					workloadv1alpha1.RevisionLabelKey:  expectedRevision,
		...
		{
			name: "one revision not match, count match",
			pods: []podSpec{
				{"pod1", map[string]string{
					workloadv1alpha1.GroupNameLabelKey: groupName,
					workloadv1alpha1.RevisionLabelKey:  expectedRevision,
				...
				{"pod2", map[string]string{
					workloadv1alpha1.GroupNameLabelKey: groupName,
					workloadv1alpha1.RevisionLabelKey:  otherRevision,
				}},
			},
			expectedCount: 2,
			want:          false,
		...
		{
			name: "all revision not match, count match",
			pods: []podSpec{
				{"pod1", map[string]string{
					workloadv1alpha1.GroupNameLabelKey: groupName,
					workloadv1alpha1.RevisionLabelKey:  otherRevision,
		...
		{
			name: "more pods than expected, all revision match",
			pods: []podSpec{
				{"pod1", map[string]string{
					workloadv1alpha1.GroupNameLabelKey: groupName,
					workloadv1alpha1.RevisionLabelKey:  expectedRevision,
		...
		{
			name: "pod missing revision label",
			pods: []podSpec{
				{"pod1", map[string]string{
					workloadv1alpha1.GroupNameLabelKey: groupName,
				}},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			got, err := c.isInferGroupPodNumExpected(group, ns, tc.expectedCount)
			assert.NoError(t, err)
			assert.Equal(t, tc.want, got)
		})
	}
