...
- **AutoScalingPolicyBinding** – Attaches policies to target workloads (e.g., ModelServing or Deployments), supporting two modes: **scalingConfiguration** for metric-driven scaling of a single target and **optimizerConfiguration** for cost-aware distribution across heterogeneous instance types and engines (e.g., H100/A100).
...
- **ModelRoute Controller** – Projects routing intent into the Kthena Router by continuously syncing matches, weights, and model/LoRA selectors from `ModelRoute` CRs. It validates and normalizes rule sets to produce the effective routing table used by the router.
- **ModelServer Controller** – Tracks backend pods via `workloadSelector`, applies traffic policies (retries, timeouts, connection settings), and publishes healthy endpoints to the router. It manages service exposure and connectivity for model-serving backends.
