...
- **AutoScalingPolicy** – Defines autoscaling rules including metric collection endpoints, scaling triggers (CPU, memory, custom metrics), scaling behaviors (rates, stabilization windows), and target thresholds for automated replica management
- **AutoScalingPolicyBinding** – Binds autoscaling policies to target workloads with two modes: **scalingConfiguration** for metric-driven scaling of individual targets, and **optimizerConfiguration** for cost-optimized workload distribution across heterogeneous hardware (e.g., H100/A100 GPU clusters)
