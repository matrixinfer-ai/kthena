...
:::info Scheduler (Gateway Component)
The Infer Gateway includes a pluggable Scheduler that plays a crucial role in the "Scheduling" stage of the request pipeline. It employs **advanced scheduling plugins** to optimize request routing and resource utilization:
...
It seamlessly integrates with the Infer Gateway’s Load Balancing and Fairness Scheduling components to ensure optimal request distribution.
:::
